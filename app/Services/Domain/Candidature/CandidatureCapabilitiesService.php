...
class CandidatureCapabilitiesService
{
    ...
    private function getDocumentsCapabilities(Candidature $candidature): array
    {
        ...
        return [
            ...
            'submitted_count' => count(array_filter($requiredDocs, fn ($doc) => $doc['statut'] !== StatutVerificationDocument::NON_SOUMIS)),
            'validated_count' => count(array_filter($requiredDocs, fn ($doc) => $doc['statut'] === StatutVerificationDocument::VALIDE)) || ($candidature->estValidee() ? count($requiredDocs) : 0),
        ];
    }
    ...
    private function getPaymentCapabilities(Candidature $candidature): array
    {
        ...
        return [
            ...
            'status' => $candidature->estValidee() ? StatutPaiement::VERIFIED->value : ($paymentCheck['status'] ?? 'UNKNOWN'),
