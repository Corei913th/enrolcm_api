...
use App\DTOs\Documents\CreateDocumentRequisDTO;
use App\DTOs\Documents\SubmitDocumentDTO;
use App\DTOs\Documents\UpdateDocumentRequisDTO;
...
class DocumentService
{
    ...
    public function validateDocument(
        ...
        return DB::transaction(function () use ($document, $dto, $userId) {
            ...
            match ($statut) {
                StatutVerificationDocument::VALIDE => $document->valider($userId, $dto->commentaire),

                StatutVerificationDocument::REJETE => $document->rejeter(
                    $userId,
     ...
     * @return Collection Liste des documents requis
     */
    ...
    public function getValidatedPhotoPath(Candidature $candidature): ?string
    {
        $photoDocument = $candidature->documents()
            ->whereHas('documentRequis', function ($q) {
                $q->where('type_document', TypeDocument::PHOTO_IDENTITE);
            })
            ->where('statut_verification', StatutVerificationDocument::VALIDE)
            ->first();
