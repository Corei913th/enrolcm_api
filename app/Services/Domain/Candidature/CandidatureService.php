...
{
  use HasAdvancedSearch, HasSmartCache, HasOptimizedUpdate;
  ...
  public function createCandidature($candidat, string $concoursId, $session, $dateInscription): Candidature
  {
    ...
    $concours = \App\Models\Concours::find($concoursId);
  ...
  public function getDashboardStats(string $candidatId): array
  {
    return $this->rememberStatic("candidat_dashboard_{$candidatId}", function () use ($candidatId) {
      $candidat = \App\Models\Candidat::where('utilisateur_id', $candidatId)->first();
      if ($candidat) {
      ...
      $alertes = \App\Models\Alert::whereIn('candidature_id', $activeCandidatureIds)
  ...
  public function completerCandidature(string $candidatureId, array $data): Candidature
  {
    ...
    if ($wasUpdated) {
      ...
      $candidature->refresh();
      try {
        $validationService = app(\App\Services\Domain\Candidature\Validators\CandidatureValidationService::class);
