...
class CandidatureValidationService
{
  ...
  public function checkAndValidateIfReady(Candidature $candidature): bool
  {
    return runTransaction(function () use ($candidature) {
      ...
      if (!\App\Helpers\CandidatureHelper::hasValidPayment($candidature->id)) {
        Log::info("Candidature {$candidature->id} payment not verified");
        return false;
      }


      if (!\App\Helpers\CandidatureHelper::hasCompleteDocuments($candidature->id)) {
        Log::info("Candidature {$candidature->id} documents not complete");
        return false;
      }

      $fieldsCheck = \App\Helpers\CandidatureHelper::hasRequiredCandidateFields($candidature->id);
  ...
  private function sendValidationNotifications(Candidature $candidature): void
  {
    ...
    \App\Models\Notification::create([
      ...
      'type_notification' => \App\Enums\TypeNotification::CANDIDATURE_VALIDEE->value,
      ...
      'canal' => \App\Enums\CanalNotification::APP->value,
      'priorite' => \App\Enums\PrioriteNotification::HAUTE->value,
      'est_lue' => false,
      'est_envoyee' => true,
      'date_envoi' => now(),
      'metadata' => [
    ...
    if ($this->notificationService->canSendEmail($candidat)) {
      try {
        ...
        \App\Models\Notification::create([
          ...
          'canal' => \App\Enums\CanalNotification::EMAIL->value,
          'priorite' => \App\Enums\PrioriteNotification::HAUTE->value,
          'est_lue' => false,
          'est_envoyee' => true,
          'date_envoi' => now(),
