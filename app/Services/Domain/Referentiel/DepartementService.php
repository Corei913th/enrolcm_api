...
class DepartementService
{
  ...
  protected string $exceptionClass = DepartementException::class;
  protected string $codeColumn = 'code_departement';
  ...
  public function create(CreateDepartementDTO $dto): Departement
  {
    try {
      $departement = Departement::create([
        ...
        'code_departement' => strtoupper($dto->code_departement),
        'libelle_departement' => $dto->libelle_departement,
        'desc_departement' => $dto->desc_departement,
        'est_actif' => $dto->est_actif ?? true,
      ]);

      $this->logCreate('departement', $departement->id, ['code' => $departement->code_departement]);
      return $departement;
  ...
  public function update(string $id, UpdateDepartementDTO $dto): Departement
  {
    ...
    try {
      $updateData = [
        ...
        'code_departement' => $dto->code_departement
      ...
      $this->updateWithCache($departement, $updateData);
      $this->logUpdate('departement', $id);
  ...
  public function delete(string $id): bool
  {
    ...
    if (hasDependencies($departement, 'centres')) {
      throw DepartementException::hasDependencies($id, 'centres');
    }

    if (hasDependencies($departement, 'filieres')) {
      throw DepartementException::hasDependencies($id, 'filières');
    }

    try {
      $result = $departement->delete();
      $this->logDelete('departement', $id);
      return $result;
    } catch (\Exception $e) {
  ...
  public function activate(string $id): Departement
  {
    ...
    $this->logStatusChange('departement', $id, true);
    return $departement->fresh();
  }
  ...
  public function deactivate(string $id): Departement
  {
    ...
    $this->logStatusChange('departement', $id, false);
    return $departement->fresh();
  }

  /**
   * Trouver un département par ID (alias pour compatibilité)
   *
   * @param string $id ID du département
  ...
  /**
   * Lister tous les départements avec recherche optimisée.
   *
   * @param array $filters Filtres optionnels
   *
   * @return LengthAwarePaginator Liste des départements
   */
  public function getAll(array $filters = []): LengthAwarePaginator
  {
    $query = Departement::query()
      ...
      ->with(['ecole:id,libelle_ecole,code_ecole']);
    ...
    if (!empty($filters['search'])) {
      $this->applySearch(
        ...
        $filters['search'],
        [
          'code_departement' => 'start',
          'libelle_departement' => 'words',
          'desc_departement' => 'partial'
        ],
        [
          'ecole.libelle_ecole' => 'partial',
          'ecole.code_ecole' => 'start'
        ]
      );
    }
    ...
    // Tri
    $sortBy = $filters['sort_by'] ?? 'code_departement';
    ...
    $this->applySort(
      ...
      $sortOrder,
      'code_departement',
      ['code_departement', 'libelle_departement', 'created_at']
    );
