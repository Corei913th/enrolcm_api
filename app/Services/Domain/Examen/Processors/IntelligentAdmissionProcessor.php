...
class IntelligentAdmissionProcessor
{
  ...
  public function process(
    ...
    // 7. Save
    $results->each(function ($result) use ($stats) {
  ...
  private function assignPlaces(
    ...
    foreach ($results as $result) {
      if ($result->categorie_admission !== CategorieAdmission::STANDARD) continue;
      if ($stats['admis'] >= $totalPlaces) break;
    ...
    if ($rule->permet_admission_conditionnelle && $stats['admis'] < $totalPlaces) {
      foreach ($results as $result) {
        if ($result->categorie_admission !== CategorieAdmission::CONDITIONNEL) continue;
        if ($result->decision === DecisionAdmission::ADMIS) continue;
        if ($stats['admis'] >= $totalPlaces || $conditionalUsed >= $maxConditional) break;
    ...
    foreach ($results as $result) {
      if ($result->decision === DecisionAdmission::ADMIS) continue;
      if ($result->categorie_admission === CategorieAdmission::ELIMINATOIRE) continue;
      if ($stats['liste_attente'] >= $waitingListSize) break;
    ...
    // Phase 4: Rejected
    foreach ($results as $result) {
      if ($result->decision !== null) continue;
