...
class CandidatureObserver
{
  ...
  {
    match (true) {
      $newStatus === StatutCandidature::VALIDE =>
      $this->notificationService->notifyCandidatureValidated($candidature->candidat, $candidature),

      $newStatus === StatutCandidature::REJETEE =>
      $this->notificationService->notifyCandidatureRejected(
        $candidature->candidat,
        $candidature,
        $candidature->motif_rejet ?? 'Candidature rejetée'
      ),

      $this->isBackwardTransition($oldStatus, $newStatus) =>
      $this->notificationService->notifyCandidatureRejected(
        $candidature->candidat,
        $candidature,
