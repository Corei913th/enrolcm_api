...
class PaiementObserver
{
  ...
  private function handlePaymentVerified(Paiement $paiement): void
  {
    ...
    if ($paiement->candidat_id) {
      \Illuminate\Support\Facades\Cache::forget("dashboard_stats_{$paiement->candidat_id}");
    }
  }
  ...
  private function handlePaymentNotification(Paiement $paiement, StatutPaiement|string|null $oldStatus, ?StatutPaiement $newStatus): void
  {
    ...
    match (true) {
      $oldEnum === StatutPaiement::OCR_VERIFIE && $newStatus === StatutPaiement::VERIFIED =>
      $this->notificationService->notifyPaymentVerified($paiement->candidat, $paiement),

      $newStatus === StatutPaiement::PENDING_MANUAL_REVIEW =>
      $this->notificationService->notifyPaymentPendingReview($paiement->candidat, $paiement),

      $oldEnum === StatutPaiement::PENDING_MANUAL_REVIEW && $newStatus === StatutPaiement::VERIFIED =>
      $this->notificationService->notifyPaymentVerified($paiement->candidat, $paiement),

      $newStatus === StatutPaiement::REJECTED =>
