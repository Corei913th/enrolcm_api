...
class TestAlertResolution extends Command
{
    ...
    private function checkAlerts($candidature)
    {
        ...
        if (in_array('profile_incomplete', $types)) {
            $this->warn(' [!] Profile Incomplete alert present');
        }
        if (in_array('missing_documents', $types)) {
            $this->warn(' [!] Missing Documents alert present');
        }
        if (in_array('payment_pending', $types)) {
            $this->warn(' [!] Payment Pending alert present');
        }
    }
}
