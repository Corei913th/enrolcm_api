...
class HasActivityLoggerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->service = new class($this->loggerMock)
        {
            ...
            public function test_log_create(string $entity, string $entityId, ?array $data = null): void
            {
                $this->logCreate($entity, $entityId, $data);
            }

            public function test_log_update(string $entity, string $entityId, ?array $data = null): void
            {
                $this->logUpdate($entity, $entityId, $data);
            }

            public function test_log_delete(string $entity, string $entityId): void
            {
                $this->logDelete($entity, $entityId);
            }

            public function test_log_view(string $entity, string $entityId): void
            {
                $this->logView($entity, $entityId);
            }

            public function test_log_status_change(string $entity, string $entityId, bool $newStatus): void
            {
                $this->logStatusChange($entity, $entityId, $newStatus);
            }

            public function test_log_operation(string $action, string $entity, ?string $entityId = null, ?array $data = null): void
            {
